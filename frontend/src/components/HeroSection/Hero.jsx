...
const HeroSection = () => {
  ...
  useEffect(() => {
    ...
    axios.get('https://musicplayer-6hmo.onrender.com/api/playlists/popular') 
      .then(response => {
  ...
  return (
    <div className='h-[90%] w-[97%] ml-4 overflow-scroll overflow-x-hidden text-white bg-gradient-to-b from-[#1F1F1F] to-[#121212]'>
      <div className="popular-artists relative">
        ...
        <p className='absolute right-0 pt-12 top-0 text-[#AAAAAA]'><Link to="/Artists">show all</Link></p>
      ...
      <div className="popular-playlists relative mt-6 ">
        <h1 className='text-2xl pt-12 ml-4 font-semibold'>Popular Playlists</h1>
        <p className='absolute right-0 pt-12 top-0 text-[#AAAAAA]'><Link to="/Playlists">show all</Link></p>
  ...
  return (
    <BrowserRouter>
      <nav className='flex w-[97%]  ml-4 justify-between bg-[#0F0F0F] rounded-t-lg items-center h-[10%]'>
        <div className="arrow flex cursor-pointer gap-2 ml-3 opacity-100">
      ...
      <Routes>
        ...
        <Route path="/search" element={<HeroSection />} />
      </Routes>
    </BrowserRouter>
  );
};
